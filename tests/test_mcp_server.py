...
def guardian_server(mock_settings: MagicMock, mock_engine: MagicMock) -> NeurosymbolicGuardianServer:
    ...
    with (
        patch("ns_guardian.mcp_server.Settings.from_env", return_value=mock_settings),
        patch("ns_guardian.mcp_server.AegisEngine", return_value=mock_engine),
    ):
...
async def test_check_logic_satisfiable() -> None:
    ...
    with (
        patch("ns_guardian.mcp_server.Settings.from_env", return_value=mock_settings),
        patch("ns_guardian.mcp_server.AegisEngine"),
        patch("ns_guardian.mcp_server.OpenAICompatibleProvider"),
    ):
        server = NeurosymbolicGuardianServer()

        # Test satisfiable constraints with proper Z3 syntax
        # Define x as an Int variable, then add constraints
        result = await server._check_logic(
            {"constraints": ["Int('x') > 0", "Int('x') < 10"], "facts": {}}
        )
