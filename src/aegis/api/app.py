...
from ..llm_providers import build_llm_client, LLMClient
...
from ..model_catalog import list_models_for_provider, get_model_info
...
class VerifyRequest(BaseModel):
    ...
    intent: Optional[Intent] = Field(None, description="Pre-extracted intent (optional)")
...
def verify_action(request: VerifyRequest) -> AegisDecision:
    ...
    try:
        ...
        if request.intent:
            ...
            intent = extract_intent_internal(request.text, request.facts)
...
def list_models(provider: Optional[LLMProvider] = None) -> ModelListResponse:
...
def list_rules() -> Dict[str, Any]:
    ...
    rules_info = []
    for rule_id in engine.registry._rules.keys():
